...
func (s *Scanner) advance() {
	...
	if r >= utf8.RuneSelf {
		...
		if r == utf8.RuneError && w == 1 {
			_ = s.makeSafePos() // called just to check validity of s.line/s.col
			s.err(token.MakePosition(s.filename, s.roff, s.line, s.col), "illegal UTF-8 encoding")
...
func (s *Scanner) makeSafePos() token.Pos {
	if s.line > token.MaxLines || s.col > token.MaxCols {
		if s.line > token.MaxLines {
			panic(fmt.Sprintf("number of lines exceeded: %d", s.line))
		}
		panic(fmt.Sprintf("number of columns exceeded at line %d: %d", s.line, s.col))
	}
